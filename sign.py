...
# 学号，部分学校可能用一卡通号等代替。可以到       https://fxgl.jx.edu.cn/你的高校代码/        自己尝试一下
ID = 123456789
...
# 如果使用输入的经纬度，即上面配置的signType = 1的话，才需要配置，否则可以忽略
# 经度
lng = 115.812811
# 纬度
lat = 28.665269
# 地址 尽量详细 包含省市区/镇
zddlwz = '江西省南昌市红谷滩区学府大道999号南昌大学前湖校区正气广场'
...
def signHistory(cookie, checkExit=False):
    if checkExit is False and signType == 1:
        global lng, lat, zddlwz
        constructionPost(lng, lat, zddlwz)
    ...
    try:
        ...
        if checkExit:
            ...
            constructionPost(last_dic["lng"], last_dic["lat"], last_dic["zddlwz"])
            return False
    except:
        if checkExit:
            return False
...
def cookieFile(cookie=None, delete=False):
    cookie_file = 'cookie/' + str(ID) + '/cookie.txt'
    if delete:
...
def constructionPost(lng1, lat1, address):
    ...
    post = "province=" + province + "&city=" + city + "&district=" + district + "&street=" + street + "&xszt=0&jkzk=0" \
                                                                                                      "&jkzkxq=&sfgl=1&gldd=&mqtw=0&mqtwxq=&zddlwz=" + address + "&sddlwz=&bprovince=" + province + "&bcity=" + city + "&bdistrict=" + district + "&bstreet=" + street + "&sprovince=" + province + "&scity=" + city + "&sdistrict=" + district + "&lng=" + str(
        lng) + "&lat=" + str(lat) + "&sfby=" + str(sfby)
    log.write(post + '\n')
    signPostInfo = post
...
if __name__ == "__main__":
    ...
    if cookieFile(cookie=cookie) is False:
        print("登陆数据文件丢失或不存在，尝试重新获取登陆数据")
        login()
    cookieFile(cookie=cookie)
    if verify(cookie) is False:
        print("尝试重新获取登陆数据")
        login()
        cookieFile(cookie=cookie)
    else:
